...
ARRAY *ARRAY *copy_string_array(ARRAY *array)     
{
  register int i;
  ARRAY *copy;
...
void extract_info (char *filename, FILE *structfile, *externfile)
{
  register int i;
  DEF_FILE *defs;
  struct stat finfo;
  ...
  if (stat (filename, &finfo) == -1)
    file_error (filename, NULL, NULL, NULL);
...
int int function_handler(char *self, DEF_FILE *defs, char *arg)  
{
  register BUILTIN_DESC *builtin;
...
int int short_doc_handler(char *self, DEF_FILE *defs, char *arg) 
{
  register BUILTIN_DESC *builtin;
...
int int comment_handler(char *self, DEF_FILE *defs, char *arg)   
{
  return (0);
}

/* How to handle the $DEPENDS_ON directive. */
int int depends_on_handler(char *self, DEF_FILE *defs, char *arg) 
{
  register BUILTIN_DESC *builtin;
  char *dependent;
...
int produces_handler (char *self, DEF_FILE *defs, char *arg)
{
  ...
  if (defs->production)
    line_error (defs, "%s already has a %s definition", defs->filename, self, NULL);
  else
    {
...
int int end_handler(char *self, DEF_FILE *defs, char *arg)
{
...
static int static int _find_in_table(const char *name, char **name_table) {
  register int i;
...
static int static int is_assignment_builtin(char *name)
{
...
static int static int is_arrayvar_builtin(char *name)
{
